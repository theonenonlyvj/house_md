...
async function openAgent(thinkModel = process.env.THINK_MODEL || 'gpt-4o-mini', isRetry = false): Promise<void> {
  ...
  ws.on('message', async (data: any, isBinary: boolean) => {
    ...
    if (msg.type === 'ConversationText') {
      ...
      if (msg.role === 'assistant' && (text.length > 500 || /SEATED COUNCIL|RULES:|argument style|^\s*[-•({[]|^\s*(Interpretation|Contradiction|Discriminator|Evidence)\b|"(aliases|personaId|spoken_line|claim|differential|rank)"\s*:|[{}\[\]]\s*$/i.test(text))) return;
